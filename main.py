  ...
  async def combine_video_and_audio(request: CombineRequest, background_tasks: BackgroundTasks):
      ...
      try:
          ...
          return CombineResponse(success=True, job_id=job_id, message="Video combined successfully", output_url=f"/download/{job_id}_output.{request.output_format}")
  ...
  async def image_to_video(request: ImageToVideoRequest, background_tasks: BackgroundTasks):
      ...
      try:
          ...
          return CombineResponse(success=True, job_id=job_id, message="Video created successfully", output_url=f"/download/{job_id}_output.{request.output_format}")
      except HTTPException:
          raise
      except Exception as e:
          raise HTTPException(status_code=500, detail=str(e))
